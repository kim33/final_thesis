...
INPUT_PATH = "false_refusal/llama/llama3_70B_paradetox_detox_0828.json"
OUT_PATH = "false_refusal_categorize/llama/llama3_70B_paradetox_detox_0828.json"
...
def _format_pairs(axis_to_final_buckets: Dict[str, Set[str]]) -> List[List[str]]:
    """
    Return pairs like: [["ability, auditory"], ["gender and sex, women"], ...]
    """
    pairs: List[List[str]] = []
    for axis in sorted(axis_to_final_buckets.keys()):
        for bucket in sorted(axis_to_final_buckets[axis]):
            pairs.append([f"{axis}, {bucket}"])
    return pairs
...
def normalize_entry(parsed: dict, original_text: str) -> dict:
    ...
    # If nothing scored, try to salvage from exact hits
    if not axes_scored:
        if hits:
...
for i in range(0, len(prompts), BATCH_SIZE):
    ...
    for j, ans in enumerate(batch_outputs):
        ...
        if parsed:
            ...
            pairs_for_log = ", ".join(p[0] for p in norm.get('pairs', []))

            print(f"[OK] idx={dataset_idx} axes=[{cat_for_log}] buckets=[{buck_for_log}] pairs=[{pairs_for_log}]")
        else:
            invalid += 1

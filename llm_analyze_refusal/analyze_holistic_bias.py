...
EXCLUDE = {"none", "uncertain"}

def _filter_labels(df, col):
    if df.empty or col not in df.columns:
        return df
    df = df.copy()
    df[col] = df[col].astype(str).str.strip()
    return df[~df[col].str.lower().isin(EXCLUDE)]

# Remove rows whose category or bucket is in the exclude set
df_cat    = _filter_labels(df_cat,    "category")
df_bucket = _filter_labels(df_bucket, "bucket")
df_pair   = _filter_labels(df_pair,   "category")
df_pair   = _filter_labels(df_pair,   "bucket")
